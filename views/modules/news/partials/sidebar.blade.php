<div class="col-md-3">
    ...
    <div class="tabs custom-tabs m-bot-30">
        <ul class="nav nav-tabs">
            <li class="active">
                <a href="#tab1" data-toggle="tab">{{ trans('themes::news.popular') }}</a>
            </li>
            <li>
                <a href="#tab2" data-toggle="tab">{{ trans('themes::news.latest') }}</a>
            </li>
        </ul>
        <div class="tab-content">
            <div id="tab1" class="tab-pane tab-pop-cat-container active">
                <ul class="latest-post-container">
                    @foreach(News::popular(3) as $popular)
        ...
        <ul class="clearfix">
            @if(Request::route()->getName() == 'news.index')
                @foreach(News::latest(10) as $recent)
                    ...
                    @if($tag)
                        <li><a class="button small thin" href="{{ route('news.tag', [$tag->slug]) }}">{{ $tag->name }}</a></li>
                    @endif
                @endforeach
            @else
